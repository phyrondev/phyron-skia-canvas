...
async function snapshot() {
  ...
  let { version } = await config(),
    releases = JSON.parse(
      (
        await exec(
          `gh api repos/phyrondev/phyron-skia-canvas/releases --paginate`,
        )
      ).stdout,
    ),
    release = releases.find((r) => r.name === `v${version}` || r.tag_name === `v${version}`);
  if (!release) {
    throw new Error(`release v${version} not found on phyrondev/phyron-skia-canvas`);
  }
  let assets = JSON.parse(
      (
        await exec(
          `gh api repos/phyrondev/phyron-skia-canvas/releases/${release.id}/assets`,
        )
      ).stdout,
    ),
    hashes = Object.fromEntries(assets.map(({ name, digest }) => [name, digest]));
  await exec(
    `npm pkg set prebuild='${JSON.stringify(hashes)}' --json`,
  );
}
...
function compile(...args) {
  ...
  let { status } = child_process.spawnSync(
    // `-nc` would derive the cdylib name from `package.json` (npm name
    // is `phyron-skia-canvas`); use the explicit `-a cdylib skia-canvas`
    // form so cargo-cp-artifact looks for the actual cargo crate name
    // (renamed for the crates.io publish; npm name unchanged).
    `cargo-cp-artifact -a cdylib skia-canvas ${BINARY_PATH} -- cargo build --message-format=json-render-diagnostics ${optimization} ${features}`,
